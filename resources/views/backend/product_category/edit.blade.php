    ...
    <section class="content">
        <div class="row">
            ...
            <div class="card card-body bg-gray-light">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" method="POST" action="{{ route('admin.product_category.update',$category->id) }}"
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" required class="form-control" name="code" id="code"
                                        placeholder="Enter Category Code" value="{{ old('code',$category->code) }}">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="group_id" id="group_id" class="form-control select2" required>
                                        ...
                                        @foreach ($groups as $group)
                                            <option value="{{ $group->id }}"
                                                @if (old('group_id',$category->group_id) == $group->id) selected @endif>{{ $group->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            @if ($category->featured_image)
                                <img src="{{ asset($category->featured_image) }}" alt="Featured Image"
                                    style="max-width: 150px; margin-top: 10px;">
                            @endif
                        </div>
                        <div class="form-group">
                            <label for="description">Description</label>
                            <textarea class="form-control" name="description" id="description" placeholder="Enter Description">{{ old('description',$category->description) }}</textarea>
                        </div>


                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1"
                                @if (old('is_active',$category->is_active) == 1) checked @endif>
                            <label class="form-check-label" for="is_active">Is Active</label>
                        </div>
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="show_as_featured" name="show_as_featured"
                                value="1" @if (old('show_as_featured', $category->show_as_featured) == 1) checked @endif>
